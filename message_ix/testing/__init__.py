...
def make_westeros(mp, emissions=False, solve=False):
    ...
    gdp_profile = np.array([1., 1.5, 1.9])
    ...
    for par, t, c, l, value in [
        ("input", "bulb", "electricity", "final", 1.),
        ("output", "bulb", "light", "useful", 1.),
        ("input", "grid", "electricity", "secondary", 1.),
        ("output", "grid", "electricity", "final", grid_efficiency),
        ("output", "coal_ppl", "electricity", "secondary", 1.),
        ("output", "wind_ppl", "electricity", "secondary", 1.),
    ]:
        scen.add_par(
            par,
            make_df(
                par, **common, technology=t, commodity=c, level=l, value=value
            )
        )

    capacity_factor = {t: 1. for t in ("coal_ppl", "wind_ppl", "bulb")}
    ...
    for tec, val in dict(
        ...
        scen.add_par(
            ...
            make_df(
                "historical_new_capacity", **common, technology=t, value=val
            )
        )
